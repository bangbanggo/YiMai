		...
		<div class="manage">
			<table class="list">
				...
				<tr>
					...
					<td class="w1 c">${comment.ecNiceName}</td>
					<td>
						<c:if test="${not empty comment.ecContent}">已回复</c:if>
					</td>
					<td class="w1 c">${comment.ecStatus}</td>
					<td class="w1 c"><a href="commentPreUpdate?comment.ecId=${comment.ecId}"><c:if test="${empty comment.ecContent}">回复</c:if></a> <a href="javascript:Delete(${comment.ecId});">删除</a></td>
				</tr>
				</c:forEach>
				<tr>
