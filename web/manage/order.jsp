		...
		<div class="manage">
			...
			<table class="list">
				...
				<tr>
					...
					<td>${order.euAddress}</td>
					<td class="w1 c">
						<c:if test="${order.eoStatus ==1}">待审核</c:if>
						<c:if test="${order.eoStatus ==2}">审核通过</c:if>
						<c:if test="${order.eoStatus ==3}">已发货</c:if>
						<c:if test="${order.eoStatus ==4}">确认收货</c:if>
					</td>
					<td class="w1 c"><a href="orderPreUpdate">修改</a></td>
				</tr>
				</c:forEach>
			</table>
			<div class="pager">
