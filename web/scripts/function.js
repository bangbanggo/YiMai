...
function CheckItem(obj)
{
	...
	switch(obj.name) {
		...
		case "user.euBirthday":
			...
			break;
		case "user.euIdentityCode":
			if(obj.value == "") {
				msgBox.innerHTML = "请填写身份信息";
				msgBox.className = "error";
				return false;
			}
			break;
		...
		case "user.euMobile":
			var reg = /^[1][0-9]{10}$/;
			if(obj.value == "" || !reg.test(obj.value)) {
				msgBox.innerHTML = "请填写正确联系方式";
				msgBox.className = "error";
				return false;
			}
			break;
		case "user.euAddress":
            var reg = /^[\u4e00-\u9fa5][\u4e00-\u9fa50-9]{0,}$/;
            if(obj.value == ""|| !reg.test(obj.value)) {
				msgBox.innerHTML = "请填写正确的住址";
				msgBox.className = "error";
				return false;
			}
			break;
...
function goBuy(id, price)
{
	...
	$.ajax({
		url:"shopping_addProdcut?product.epId="+id,
...
function submitOrder(){
	var doms = document.getElementsByName("number");
	var result = "";
	for(var i=0;i<doms.length;i++){
		var pro = doms[i];
		var strs = pro.id.split("_");
		var id = strs[2];
		result+=id+"/"+pro.value+",";
	}
	setCookie("order",result.substr(0,result.length-1));
	location.href="order_submit";
}